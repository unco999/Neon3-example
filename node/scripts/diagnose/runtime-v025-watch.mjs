// v0.2.5: spawn all three, then poll the wgpu process liveness + capture exit cause.
import { spawn } from "node:child_process";
import { NeonClient } from "@neon3/sdk";

const emit = (value) => process.stdout.write(JSON.stringify(value) + "\n");
const V025 = "C:/Users/10540/AppData/Local/Neon3Sdk/runtime/v0.2.5";
const ports = { eventd: "127.0.0.1:39501", ui: "127.0.0.1:39502", wgpu: "127.0.0.1:39503", domain: "127.0.0.1:39504" };
const children = {};
...
const healthy = async (endpoint, target) => {
  ...
  return r.status === "accepted" && r.result?.status === "healthy";
};

spawnProc("neon-eventd", ["--server", ports.eventd, "1"]);
await new Promise((r) => setTimeout(r, 800));
spawnProc("neon-wgpu-runtime", ["--window-server", ports.wgpu, ports.ui, "--eventd", ports.eventd]);
await new Promise((r) => setTimeout(r, 800));
spawnProc("neon-ui-runtime", ["--forward-server", ports.ui, ports.wgpu, ports.domain, "--eventd", ports.eventd]);

for (let i = 1; i <= 20; i += 1) {
  await new Promise((r) => setTimeout(r, 500));
  const alive = Object.entries(children).map(([name, child]) => `${name}:${child.exitCode === null ? "up" : "dead(" + child.exitCode + "/" + child.signalCode + ")"}`).join(" ");
  let h = "";
  try { h = await healthy(ports.wgpu, "wgpu-runtime") ? "healthy" : "unhealthy"; } catch (e) { h = `err:${String(e?.message ?? e).slice(0, 40)}`; }
