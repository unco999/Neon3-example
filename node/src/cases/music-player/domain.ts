...
export function stateOf(state: MusicPlayerState) { return { current_track: state.current_track, is_playing: state.is_playing, is_paused: !state.is_playing, position: state.position, volume: state.volume, shuffle: state.shuffle, repeat: state.repeat, violet_liked: Boolean(state.tracks.find((item) => item.key === "violet-orbit")?.liked) }; }
